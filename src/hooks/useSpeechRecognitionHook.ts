...
const useSpeechRecognition = () => {
  ...
  const fetchTranslation = async (recognizedText: string, targetLanguage: string) => {
    try {
      const apiUrl = process.env.REACT_APP_TRANSLATE_API_URL || 'http://localhost:5001/translate'; // Use environment variable
  ...
  useEffect(() => {
    ...
    recognitition.onerror = (event: any) => {
    ...
    recognitition.onend = () => {
      ...
      if (isListening) {
        recognitition.start(); // Restart recognition if still listening
      }
    };
  }, [isListening]);
